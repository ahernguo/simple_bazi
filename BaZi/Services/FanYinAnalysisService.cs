    ...
    public sealed class FanYinAnalysisService {
        ...
        public FanYinAnalysisResult AnalyzeNatalPillars(
            ...
            for (var i = 0; i < pillars.Count; i++) {
                for (var j = 0; j < pillars.Count; j++) {
                    if ((i == j) || found.Any(pair => pair.Contains(i) && pair.Contains(j))) {
                        continue;
                    }
                    if (IsFanYinGan(pillars[i].Gan, pillars[j].Gan) && IsFanYinZhi(pillars[i].Zhi, pillars[j].Zhi)) {
